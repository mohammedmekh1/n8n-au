...
export type WeddingOrder = {
  order_id: string; groom_name: string; bride_name: string; phone: string
  wedding_date: string; package_type: string; status: string; created_at: string
}

export type FilteredLead = {
  raw_text: string; reason: string; platform: string; captured_at: string
}
...
export async function getWeddingOrders(limit = 50): Promise<WeddingOrder[]> {
  const rows = await fetchSheet('wedding_orders!A:AD')
  return rowsToObjects<WeddingOrder>(rows).slice(-limit)
}

export async function getFilteredLeads(limit = 100): Promise<FilteredLead[]> {
  const rows = await fetchSheet('filtered_leads!A:AD')
  return rowsToObjects<FilteredLead>(rows).slice(-limit)
}

// ── Computed KPIs ──────────────────────────────────────────────────────────
export async function getDashboardKPIs() {
  const [leads, reports, tasks, posts, articles, weddings, filtered] = await Promise.all([
    getLeads(500), getDailyReports(7), getSalesTasks(200),
    getSocialPosts(100), getArticles(50), getWeddingOrders(50),
    getFilteredLeads(100)
  ])
  ...
  return {
    ...
    weddingRecent:     weddings.slice(-5),
  }
}
