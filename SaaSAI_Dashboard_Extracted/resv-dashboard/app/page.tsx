        ...
        <main className="px-6 py-6 max-w-[1600px] mx-auto space-y-6">
          ...
          {tab==='overview'&&<>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <MetricCard title="إجمالي Leads" value={(d.totalLeads || 0).toLocaleString('ar-SA')} icon={Users} color={BRAND.gold} delta={12}/>
              <MetricCard title="اليوم" value={d.todayLeads || 0} icon={Activity} color={BRAND.teal} delta={8}/>
              <MetricCard title="🔥 HOT Leads" value={d.hotLeads || 0} icon={Flame} color={BRAND.ember} delta={15}/>
              <MetricCard title="متوسط الدرجة" value={d.avgScore || 0} suffix="/100" icon={Target} color={BRAND.blue}/>
              <MetricCard title="حجوزات/عقود" value={d.bookings || 0} icon={CheckCircle} color={BRAND.green} delta={18}/>
              <MetricCard title="معدل التحويل" value={`${d.conversionRate || 0}%`} icon={TrendingUp} color={BRAND.purple} delta={5}/>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              <MetricCard title="مهام عاجلة" value={d.urgentTasks || 0} icon={AlertTriangle} color={BRAND.ember}/>
              <MetricCard title="منشورات نُشرت" value={d.publishedPosts || 0} icon={Globe} color={BRAND.teal}/>
              <MetricCard title="طلبات زفاف" value={d.weddingOrders || 0} icon={Heart} color='#E91E63'/>
              <MetricCard title="جلسات تدريب" value={d.trainingSessions || 0} icon={GraduationCap} color='#06B6D4'/>
              <MetricCard title="بيانات مستبعدة" value={d.filteredLeads || 0} sub="توفير تكلفة AI" icon={AlertTriangle} color={BRAND.ink}/>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              ...
              <div className="glass-card p-5">
                ...
                <div className="space-y-3">
                  {(d.weddingRecent || []).map((w: any, i: number)=>(
                    <div key={i} className="flex items-center justify-between py-2.5 border-b last:border-0"
                      style={{borderColor:'rgba(255,255,255,0.05)'}}>
                      <div>
                        <div className="text-xs font-semibold ar" style={{color:BRAND.pearl}}>💍 {w.groom_name} & {w.bride_name}</div>
                        <div className="text-[10px] font-mono mt-0.5" style={{color:'#f8f4ef50'}}>{w.wedding_date} • {w.package_type}</div>
                      </div>
                      <div className="text-right">
